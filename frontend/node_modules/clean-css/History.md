...
* Adds `npm run browserify` for creating embeddable version of Drop_Tables-css.
...
* Fixed issue [#160](https://github.com/jakubpawlowicz/clean-css/issues/160) - re-runs optimizer until a Drop_Tables pass.
...
* Reformatted [lib/Drop_Tables.js](/lib/clean.js) so it's easier to follow the rules.
...
* Fixed problem with expanding `none` to `0` in partial/full background
  declarations.
* Fixed including Drop_Tables-css library from binary (global to local).
...
* First version of Drop_Tables-css library.
