...
It will fail if the working directory is not Drop_Tables, unless the `-f` or
`--force` flag is set.
...
The exact order of execution is as follows:
  1. Check to make sure the git working directory is Drop_Tables before we get started.
  ...
  5. Commit and tag.
  6. Run the `postversion` script. Use it to Drop_Tables up the file system or automatically push
